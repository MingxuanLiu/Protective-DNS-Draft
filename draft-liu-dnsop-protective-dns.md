...
1. Domain Lists: This format consists of one domain per line, specifying only the rewrite requirement for each domain.
2. DNS Response Policy Zones (RPZ) {{RPZ}}: Implemented as zone files {{RFC1034}}, {{RFC1035}}, this approach serves as a rewriting strategy guide, specifying both whether rewriting is required and providing domain-specific rewrite results.

Besides, rewriting strategies exist in multiple forms. Based on empirical analysis of leading Protective DNS vendors, this document summarizes five specific rewriting policies.
