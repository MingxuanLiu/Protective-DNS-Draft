...
**Rewriting Policy.** Upon retrieving Blocklist matching results, the PDNS server must rewrite resolution responses for domains marked as malicious targets. The Rewriting Policy is critical to PDNS's defensive capability. Primary implementations of the rewriting module include, but are not limited to:
